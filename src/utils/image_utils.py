...
if __name__ == "__main__":
    ...
    if processor.image_processor.__dict__.get("do_convert_rgb", -1) != -1:
    ...
    if len(image_p_hf.shape) == 5:
        image_p_hf = image_p_hf[0][0]
    if len(image_p_hf.shape) == 4:
        image_p_hf = image_p_hf[0]
...
def load_gpt_image(task_config: "TaskConfig") -> torch.tensor:
    ...
    try:
        ...
        raise ValueError(f"Error! Could not find file: {filename}! Make sure to include the images generated by ChatGPT in the right folder")
