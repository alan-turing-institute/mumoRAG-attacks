...
class EmbeddedDataset:
    ...
    def compute_embeddings(self, batch_size=None, for_queries=True, for_images=True):
        if for_images:
            ...
            if batch_size is None:
                batch_size = len(self.dataset.images)
            t = time.time()
            img_embeds = [self.embedder.compute_img_embedding(self.dataset.images[i * batch_size : (i + 1) * batch_size], None) for i in tqdm(range(math.ceil(len(self.dataset.images) / batch_size)))]
    ...
    def evaluate_retrieval(self, ks: list[int], loss_types: list[EmbeddingLoss], is_targeted: bool, target_query_idx, include_adv=True):
        ...
        for loss_type, k in loss_types_and_topks:
            ...
            retrievals[f"loss_{loss_type}"] = {"train": topk.indices[: self.dataset.num_train], "test": topk.indices[self.dataset.num_train :]}
